...
**採用方針**:
- WSL2 / YOUR_SERVER_IP で `uv tool install git+https://github.com/kazyam53/openai_gen_image_mcp.git` で同方式インストール。
...
**判明事実**:
- ソースは Windows 側に完備: `C:/Users/youruser/Documents/Program/claude-image-tools/mcp_excalidraw/` に `src/` `dist/` `Dockerfile` `Dockerfile.canvas` `docker-compose.yml` `package.json` が揃っている。
...
**採用方針**:
- 既存 compose をそのまま流用。YOUR_SERVER_IP にコピー → `docker compose up -d` で稼働。
...
**採用方針**:
- WSL2 / YOUR_SERVER_IP で `npm install -g claude-mermaid` で同方式インストール。
...
## 0-4 `YOUR_SERVER_IP` の常駐スタック + 既存使用ポート
...
- Caddy 設定ファイル: **Caddy コンテナ内の `/etc/caddy/Caddyfile`** が真の設定 (ConnectC2X の compose で `./Caddyfile:/etc/caddy/Caddyfile:ro` として mount されているはず → 編集対象は `/path/to/ConnectC2X/Caddyfile` か別の真ファイル要確認)

**採用方針**:
- 新規 image-hub サービスは `/path/to/image-hub/` に独立 compose を作る (既存パターン踏襲)
...
**判明事実**:
- x-api が `https://your-host.example.com/mcp` で稼働中 = サブドメインは既に確保済み (your-host.example.com + 直下 `/mcp` パス)
- TLS 取得済み (HTTPS 接続成立)
- reverse proxy は **Caddy** (`via: 1.1 Caddy`)
- HSTS 有効、Strict-Transport-Security: max-age=31536000

**採用方針 (確定、ユーザー指定)**:
- **`your-host.example.com` 直下のパスベース (`/mcp/openai-image` 等) は禁止**。理由: 既存の X-MCP / IP-MCP / Relay-MCP は同様にパスベースで揃えようとして失敗した実績あり (全部 ConnectX2C に吸い込まれた)。X-MCP / IP-MCP / Relay-MCP が個別サブドメインに分かれているのはこのため。
- **独立サブドメインを必ず取る**。例: `image-hub.your-host.example.com` (実際の名前は Phase 0-11 でユーザー確定)。
...
**判明事実 (YOUR_SERVER_IP SSH 調査 + Caddy adapt 結果より、決定的)**:
- 認可サーバーは **ConnectC2X** (`connect-c2x:3000` / `YOUR_SERVER_IP:3001`、`/path/to/ConnectC2X/` で稼働中、ユーザー自作)
- Caddy 経由 `your-host.example.com` で次の OAuth 2.1 エンドポイントを公開:
...
- 既存サブドメイン MCP (`relay.your-host.example.com` / `ipmcp.your-host.example.com`) は **同じ ConnectC2X の OAuth を共有して使っている前提** (Caddy の routing から確認可能)
...
**採用方針**: Relay-MCP の `auth.ts` をベース流用、audience を `https://image-hub.your-host.example.com` に差し替えて image-hub に組み込む。完全独立で ConnectC2X 非依存。MCP 2025-06-18 + RFC 8707 + RFC 9728 すべて満たす。

**Phase 2.A.D-5 (OAuth gate 実装) のコスト見積**: Relay-MCP の auth.ts コピー + audience 設定差し替えで約 1 日。改修ではなくパターン流用なので低リスク。

**採用方針**:
- image-hub も ConnectC2X の OAuth を流用。`image-hub.your-host.example.com` を新規 audience として ConnectC2X に登録できれば最短。
...
- 0-11: `image-hub.your-host.example.com` サブドメイン取得、Caddy ホストブロック追加 (TLS 自動)
- 0-12: Relay-MCP の自前 OAuth 2.1 (auth.ts 613 行) を流用、audience を image-hub に差し替え → MCP 2025-06-18 + RFC 8707 + RFC 9728 完全準拠

### 要ユーザー確認 (黄、残り 1 つ)
- 0-1 続き: GitHub `kazyam53/openai_gen_image_mcp` の `uv tool install` を YOUR_SERVER_IP で実行 OK? (現行 Windows と同じソースの継続)
