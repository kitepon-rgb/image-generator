...
- 実機デプロイ先: `kite@192.168.1.2:/home/kite/image-hub/`、公開 URL `https://image-hub.kitepon.dev`

## What this project is about

Windows 側に stdio で登録されていた画像/作図系 MCP サーバー 3 本 (`openai-image` / `excalidraw` / `mermaid`) を、LAN サーバー `192.168.1.2` の Docker Compose 上に集約し、`image-hub.kitepon.dev` サブドメインから OAuth 2.1 経由で Windows / WSL2 / 他 PC から共有利用する。
...
- **proxy intercept で生成物を `/files/<id>` URL に rewrite** (2026-05-04 追加): openai-image MCP のような「コンテナ内 tmp に生成物を保存して絶対パスを返す」 上流に対応するため、`/mcp/<name>` proxy が `tools/call` レスポンスを streaming SSE Transform で chunk 単位 parse → text content 中の `/var/lib/openai-image-tmp/openai_gen_image_*/...` を検出 → SHA256 prefix12 を id に storage 格上げ + artifacts DB 登録 → text を `https://image-hub.kitepon.dev/files/<id>.<ext>` URL に rewrite して re-emit。`Image` content (base64) は不変 (LLM が画像を見て会話できる必要があるため)。 走査対象は `result.content[]` と `result.structuredContent.result[]` の両方 (後者は fastmcp 系上流が content[] と並行して mirror するため、 2026-05-18 追加)。実装は [server/image-hub-app/src/intercept.ts](server/image-hub-app/src/intercept.ts) の `REWRITE_RULES` (現状 openai-image のみ、 mermaid・excalidraw は path pattern 確認後に追加)。openai-image-mcp は `TMPDIR=/var/lib/openai-image-tmp` を named volume `openai-image-tmp` で image-hub-app と共有 (ro マウント) する構成 ([compose.yml](server/compose.yml))。Python `tempfile.mkdtemp` が固定 0o700 で dir 作成するため、新しい [openai-image-mcp/server.py](server/openai-image-mcp/server.py) は mkdtemp 直後に `os.chmod(sub, 0o755)` を明示的に呼ぶ (image-hub-app uid 1000 から ro 参照するため)。旧構成の sitecustomize.py monkey-patch は不要になり削除済。
